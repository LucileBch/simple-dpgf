...
import { addRequestParams } from "../services/http-service";
import { TokenContext } from "../contexts/token-context";
import { apiEndpoints } from "../appConstants";
import { setTokensInCookies, removeCookies, removeUserFromLocalStorage, } from "../services/authentication-service";
import { AlertContext } from "../contexts/alert-context";
export function useHttp() {
    const { accessToken, setAccessToken, setRefreshToken, setIsAuthenticated } = useContext(TokenContext);
    ...
    // );
    const handleRetryWithRefreshToken = useCallback(async (response, finalUrl, headers, options) => {
        if (response.status === 401) {
            try {
    ...
    return useMemo(() => ({
        async get(url, requestParams) {
            const { finalUrl, headers } = addRequestParams(url, accessToken, requestParams);
            const options = {
                method: "GET",
                headers,
            };
            try {
                ...
                return handleRetryWithRefreshToken(response, finalUrl, headers, options);
            }
            catch (error) {
        ...
        async post(url, payload, requestParams, customOptions) {
            const { finalUrl, headers } = addRequestParams(url, accessToken, requestParams);
            ...
            try {
                const response = await fetch(finalUrl, options);
                if (response.status === 401) {
                    const newResponse = handleRetryWithRefreshToken(response, finalUrl, headers, options);
                    return newResponse;
                }
                if (!response.ok) {
        ...
        async put(url, payload, requestParams) {
            const { finalUrl, headers } = addRequestParams(url, accessToken, requestParams);
            headers.set("Content-Type", "application/json");
            const options = {
                method: "PUT",
            ...
            try {
                const response = await fetch(finalUrl, options);
                if (response.status === 401) {
                    const newResponse = await handleRetryWithRefreshToken(response, finalUrl, headers, options);
                    return newResponse;
                }
                if (!response.ok) {
                    const errorMessage = await response.text();
        ...
        async deleteRequest(url, requestParams) {
            const { finalUrl, headers } = addRequestParams(url, accessToken, requestParams);
            headers.set("Content-Type", "application/json");
            const options = {
                method: "DELETE",
