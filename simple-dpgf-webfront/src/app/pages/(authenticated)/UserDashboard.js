...
import { removeCookies, removeUserFromLocalStorage, } from "../../core/services/authentication-service";
import { TokenContext } from "../../core/contexts/token-context";
export default function UserDashboard() {
    const navigate = useNavigate();
    const location = useLocation();
    const { user, setUser } = useContext(UserContext);
    const { setAccessToken, setRefreshToken, setIsAuthenticated } = useContext(TokenContext);
    useEffect(() => {
        if (!user) {
        ...
        const roleBaseRedirection = {
            ...
            [RoleEnum.PROJECT_OWNER]: pagesUrl.MOA_PROJECTS_PAGE,
        };
